		...
		while (true) {
			...
			try {
				...
				if (request instanceof ImageRequest) {
					...
					int type = imageRequest.getSourceType();
					if (type == SourceType.SOURCE_TYPE_WEB) {
						imageRequest.addMarker("network-queue-take");
						if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
							TrafficStats.setThreadStatsTag(request
									.getTrafficStatsTag());
						}
						...
						if (networkResponse.notModified
								&& imageRequest.hasHadResponseDelivered()) {
						...
						if (request.shouldCache()
							...
							mCache.put(request.getCacheKey(),
									response.cacheEntry);
							request.addMarker("network-cache-written");
						}
					} else if (type == SourceType.SOURCE_TYPE_RES) {
						response = imageRequest
								.parResResponse(request.getUrl());
					} else if (type == SourceType.SOURCE_TYPE_FILE) {
						response = imageRequest.parLocalFileResponse(request
								.getUrl());
					}
				} else {
					...
					// If the server returned 304 AND we delivered a
					// response
					// already,
					// we're done -- don't deliver a second identical
					// response.
					if (networkResponse.notModified
				...
				VolleyLog.e(throwable, "Unhandled exception %s",
						throwable.toString());
				mDelivery.postError(request, new VolleyError(throwable));
			}
		}
	}
