...
public class ImageLoader {
	...
	public interface ImageCache {
		...
		public void putBitmapDrawable(String url, BitmapDrawable bitmap);
	...
	public boolean isCached(String requestUrl, int maxWidth, int maxHeight) {
		...
		return mCache.getBitmapDrawable(cacheKey) != null;
	}
	 ...
	 *            loaded.
	 */
	public ImageContainer get(String requestUrl, int sourceType,
			final ImageListener listener) {
		return get(requestUrl, sourceType, listener, 0, 0, false);
	}
	...
	public ImageContainer get(String requestUrl, int sourceType,
		...
		BitmapDrawable cachedBitmapDrawable = mCache
				.getBitmapDrawable(cacheKey);

		if (cachedBitmapDrawable != null) {
			// Return the cached bitmap.
			if (!isForce) {
				ImageContainer container = new ImageContainer(
						cachedBitmapDrawable, requestUrl, sourceType, null,
						null);
				imageListener.onResponse(container, true);
				return container;
			} else {
				cachedBitmapDrawable = null;
				mCache.removeItem(cacheKey);
			}
		}

		// The bitmap did not exist in the cache, fetch it!
		ImageContainer imageContainer = new ImageContainer(null, requestUrl,
				sourceType, cacheKey, imageListener);
		...
		// network and
		// track it.
		Request<?> newRequest = new ImageRequest(requestUrl, sourceType,
				new Listener<Bitmap>() {
					@Override
	...
	private void onGetImageSuccess(String cacheKey, Bitmap response) {
		...
		if (response != null) {
			if (Utils.hasHoneycomb()) {
				...
				// BitmapDrawable
				bitmapDrawable = new BitmapDrawable(mResources, response);
			} else {
			...
			if (mCache != null) {
				mCache.putBitmapDrawable(cacheKey, bitmapDrawable);
			}
		}
	...
	public class ImageContainer {
		...
		public ImageContainer(BitmapDrawable bitmapDrawable, String requestUrl,
				int sourceType, String cacheKey, ImageListener listener) {
		...
		/**
		 * Returns the bitmapdrawable associated with the request URL if it has
		 * been loaded, null otherwise.
		 */
