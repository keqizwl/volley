...
public class ImageRequest extends Request<Bitmap> {
	 ...
	 */
	public ImageRequest(String url, int sourceType,
			Response.Listener<Bitmap> listener, int maxWidth, int maxHeight,
			Config decodeConfig, Response.ErrorListener errorListener, Resources resources) {
		super(Method.GET, url, sourceType, errorListener, resources);
	...
	public Response<Bitmap> parLocalFileResponse(String filePath) {
		...
		if (bitmap == null) {
			return Response.error(new VolleyError("parse file error"));
		} else {
			return Response.success(bitmap, null);
		}
	}

	public Response<Bitmap> parResResponse(String resIdStr) {
		int resId = 0;
		try {
			resId = Integer.parseInt(resIdStr);
		} catch (Exception e) {
			return Response.error(new VolleyError("resId not exit"));
		}
		BitmapFactory.Options decodeOptions = new BitmapFactory.Options();
		Bitmap bitmap = null;
		if (mMaxWidth == 0 && mMaxHeight == 0) {
			...
			// If we have to resize this image, first get the natural bounds.
			decodeOptions.inJustDecodeBounds = true;
			BitmapFactory.decodeResource(getResources(), resId, decodeOptions);
			int actualWidth = decodeOptions.outWidth;
			int actualHeight = decodeOptions.outHeight;
